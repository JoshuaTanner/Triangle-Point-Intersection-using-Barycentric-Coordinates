...
void CScene::PointTriangleIntersect()
{
	...
	Point v0(pTriangle->vecPoints[2].x - pTriangle->vecPoints[0].x, pTriangle->vecPoints[2].y - pTriangle->vecPoints[0].y);
	Point v1(pTriangle->vecPoints[1].x - pTriangle->vecPoints[0].x, pTriangle->vecPoints[1].y - pTriangle->vecPoints[0].y);
	Point v2(pPoint->x - pTriangle->vecPoints[0].x, pPoint->y - pTriangle->vecPoints[0].y);

	float u, v;

	u = (DotProduct(v1, v1) * DotProduct(v2, v0) - DotProduct(v1, v0)*DotProduct(v2, v1)) / 
		(DotProduct(v0, v0)*DotProduct(v1, v1) - DotProduct(v0, v1)*DotProduct(v1, v0));

	v = (DotProduct(v0, v0) * DotProduct(v2, v1) - DotProduct(v0, v1)*DotProduct(v2, v0)) /
		(DotProduct(v0, v0)*DotProduct(v1, v1) - DotProduct(v0, v1)*DotProduct(v1, v0));
...
void CScene::Draw()
{
	...
	if (pPoint != NULL && CurrentMode == FINISHED){
		HPEN pointPen = CreatePen(PS_DASH, 10, RGB(0.0f,0.0f,1.0f));
